...
//  size_t i;
  ...
  double x0=-10, x1=-5, epsabs=0, epsrel = 1e-3;
  double y0=-10, y1=-5;
  ...
  double y_init[2] = {y0, y1};
  gsl_vector *x = gsl_vector_alloc (n);
  gsl_vector *y = gsl_vector_alloc (n);

  gsl_vector_set (x, 0, x_init[0]);
  gsl_vector_set (x, 1, x_init[1]);
  gsl_vector_set (y, 0, y_init[0]);
  gsl_vector_set (y, 1, y_init[1]);
...
//    print_state (iter, s);
    ...
    status =
            gsl_multiroot_test_delta(y, x, epsabs, epsrel);
  }
