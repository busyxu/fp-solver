...
*/
  gsl_vector_float_view X = gsl_vector_float_view_array(a, 3);
  gsl_matrix_float_view A = gsl_matrix_float_view_array(c, 3, 3);

  /* Compute C = A B */

  int status = gsl_blas_strmv(CblasUpper, CblasNoTrans, CblasNonUnit, &A.matrix, &X.vector);
  printf("%d\n", status);
