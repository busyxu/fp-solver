...
int main (void)
{
  ...
  float b[] = { 1011, 1012, 1021};

  float c[] = { 0.21, 0.22, 0.23,
                 0.00, 0.00, 1022,
                 1031, 1032, 1111 };
...
*/
  gsl_vector_float_view A = gsl_vector_float_view_array(a, 3);
  gsl_vector_float_view B = gsl_vector_float_view_array(b, 3);
  gsl_matrix_float_view C = gsl_matrix_float_view_array(c, 3, 3);

  /* Compute C = A B */

  int status = gsl_blas_sgemv(CblasNoTrans, alpha, &C.matrix, &A.vector, beta, &B.vector);
  printf("%d\n", status);
  //printf ("[ %g, %g\n", c[0], c[1]);
  //printf ("  %g, %g ]\n", c[2], c[3]);
