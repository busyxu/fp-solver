...
*/
  gsl_vector_complex_float_view X = gsl_vector_complex_float_view_array(a, 3);
  gsl_matrix_complex_float_view A = gsl_matrix_complex_float_view_array(c, 3, 3);

  /* Compute C = A B */

  int status = gsl_blas_ctrsv(CblasUpper, CblasNoTrans, CblasNonUnit, &A.matrix, &X.vector);
  printf("%d\n", status);
  //printf ("[ %g, %g\n", c[0], c[1]);
  //printf ("  %g, %g ]\n", c[2], c[3]);
