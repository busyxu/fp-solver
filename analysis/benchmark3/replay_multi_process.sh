...
#      cover_res=$(python3 ${GET_FUN_INFO_PY} ${driver_name} ${PWD}/${obj_gcov} ${PWD}/"temp.out")
      ...
      /home/aaa/fp-solver/llvm-6/install/bin/llvm-cov gcov ${gcov_file_path} -b > temp_branch.out
      cat temp_branch.out
      cover_branch_res=$(python3 ${GET_BR_INFO_PY} ${driver_name} ${gcov_res_path} )
...
#    echo  ${fin_cover_line_res}
    echo "===>final cover line res:"${cover_line_res}
#    rm temp.out

    # get total branch info
    /home/aaa/fp-solver/llvm-6/install/bin/llvm-cov gcov ${gcov_file_path} -b > temp_branch.out
    echo "cat temp.out (gcov_res):"
    cat temp_branch.out
    fin_cover_branch_res=$(python3 ${GET_BR_INFO_PY} ${driver_name} ${gcov_res_path} )
