...
def drawfigure(incsvfile, outfilename):
    ...
    with open("csv_line_1/"+incsvfile, 'r') as f:
        for line in f:
            if "gsl_" not in line:
                continue
    ...
    mergedList = [max(values) for values in zip(z3List, bitwuzlaList, mathsat5List, drealList, cvc5List, fp2intList, jfsList, gosatList, synergyList)]
...
drawfigure("algorithm_bfs_dict_output.csv", "algorithm_bfs_box.pdf")
drawfigure("algorithm_dfs_dict_output.csv", "algorithm_dfs_box.pdf")
