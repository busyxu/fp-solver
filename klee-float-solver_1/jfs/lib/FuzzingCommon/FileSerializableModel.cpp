...
// jfs add by zgf : if get result from Fuzzing, we get it
Z3ASTHandle loadArrayBitVector(JFSContext& ctx, const llvm::MemoryBuffer* mb,
                               Z3SortHandle sort, unsigned startBit,
                               unsigned endBit, uint64_t arrayWidth) {
  assert(sort.isArrayTy());
  if (!accessInRange(mb, startBit, endBit)) {
    ctx.raiseFatalError("load out of range of buffer");
    return Z3ASTHandle();
  }
  auto bitWidth = arrayWidth; // In bits
  assert(bitWidth == (endBit - startBit + 1));
  assert(bitWidth > 0);
  if (bitWidth > 64) {
    // TODO: When we support larger sorts add support here to load them
  ...
  jfs_nr_bitvector_ty rawBits = jfs_nr_make_bitvector(
      ...
      /*lowBit=*/startBit,
      /*highBit=*/endBit);
  Z3SortHandle bvSort = Z3SortHandle::getBitVectorTy(ctx.getZ3Ctx(), bitWidth);
  return Z3ASTHandle(Z3_mk_unsigned_int64(ctx.getZ3Ctx(), rawBits, bvSort),
                     ctx.getZ3Ctx());
}
...
bool saveArrayBitVector(JFSContext& ctx, llvm::FileOutputBuffer* buf,
                   Z3SortHandle sort, Z3AppHandle assignment, unsigned startBit,
                   unsigned bitWidth) {
  assert(sort.isArrayTy());
  if (!accessInRange(buf, startBit, bitWidth)) {
...
FileSerializableModel::loadFrom(const llvm::MemoryBuffer* buf,
  ...
  for (const auto& be : *ba) {
    ...
    case Z3_ARRAY_SORT:{
      uint64_t arrayWidth = be.getTypeBitWidth();
      assignmentExpr =
          loadArrayBitVector(ctx, buf, be.getSort(),
                             currentBufferBit, endBufferBit,arrayWidth);
      break;
    }
...
bool FileSerializableModel::saveTo(llvm::FileOutputBuffer* buf,
  ...
  for (const auto& be : *ba) {
    ...
    // jfs add by zgf to support array
    case Z3_ARRAY_SORT:{
      success = saveArrayBitVector(ctx, buf, be.getSort(), assignment.asApp(),
                              currentBufferBit, bitWidth);
      break;
    }
    default:
