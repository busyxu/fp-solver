...
llvm::cl::opt<unsigned> MaxNumSeed(
    ...
    llvm::cl::init(3), llvm::cl::cat(jfs::fuzzingCommon::CommandLineCategory));
...
buildSeedManagerOptionsFromCmdLineWithSeed(std::map<std::string,uint64_t> fuzzSeed) {
  ...
  if (AddAllZerosSeed) {
    auto abeGen = std::unique_ptr<SeedGenerator>(
            new AllBytesEqualGenerator("zeros", 0));
    opts->generators.push_back(std::move(abeGen));
  }

  if (AddAllOnesSeed) {
    auto abeGen = std::unique_ptr<SeedGenerator>(
            new AllBytesEqualGenerator("ones", 0xff));
    opts->generators.push_back(std::move(abeGen));
  }
  ...
  auto drealSeedGen = std::unique_ptr<SeedGenerator>(
          new DrealFuzzSeedGenerator("dreal-seed",fuzzSeed));
  opts->generators.push_back(std::move(drealSeedGen));
