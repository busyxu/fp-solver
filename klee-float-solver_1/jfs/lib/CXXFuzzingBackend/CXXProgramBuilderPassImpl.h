...
// jfs add by zgf to model const array class info
class BvArray{
public:
  const jfs::core::Z3ASTHandle declApp;
  std::string name;
  int range;  // 'range' is usually 8
  int width;  // 'width' decided by maxSelectSize
  std::vector<std::string> selectAssignment;
  ...
  int getBytes() const{ return width;}
  // totalBits = range * width
  int getTotalBits() const { return (int)(range*width);}
  std::string getName() const { return name; }

  void pushSelectAssignment(const std::string& ss){ selectAssignment.push_back(ss);}

  BvArray(const jfs::core::Z3ASTHandle declApp,unsigned range):
    range(range),width(0),declApp(declApp){
    name = declApp.toStr();
  }
...
private:
  ...
  // jfs add by zgf to record array max select size
  std::map<jfs::core::Z3ASTHandle,int> constArrayBytes;
  std::map<jfs::core::Z3ASTHandle,uint64_t> arrayVarWidth;
  // jfs add by zgf to support uninterpreted function
  std::vector<jfs::core::Z3ASTHandle> uninterpretedFuncVar;
  std::map<std::string ,klee::FunctionTypeInfo> functionsTypeInfo;
  std::map<std::string ,klee::FunctionTypeInfo>::iterator funcIter;
  std::string JFS2ActualType(jfs::core::Z3ASTHandle e,
                             const klee::ParameterTypeInfo& paraInfo,
                             const std::string& argName);
  std::string Actual2JFSType(jfs::core::Z3ASTHandle e,
  ...
  CXXTypeRef getOrInsertTy(jfs::core::Z3ASTHandle sort);
  ...
  void insertConstantArrayAssignments(
      CXXCodeBlockRef cb,
      const std::vector<jfs::core::Z3ASTHandle>& constraint);
  void insertUninterpretedFuncDecl();
  std::string getUninterpretedFuncUniqName(jfs::core::Z3ASTHandle funcVar);
  ...
  // jfs add by zgf to support 'select/store'
  void visitBvSelect(jfs::core::Z3AppHandle e) override;
  void visitBvStore(jfs::core::Z3AppHandle e) override;
  ...
  // jfs add by zgf
  void
  visitConvertToIEEEBitVectorFromFloat(jfs::core::Z3AppHandle e) override;
};
}
}
#endif
