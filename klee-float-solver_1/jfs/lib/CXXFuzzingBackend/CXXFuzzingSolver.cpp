  ...
  bool sortsAreSupported(Query& q) {
    ...
    auto p = std::make_shared<SortConformanceCheckPass>([&ctx](Z3SortHandle s) {
      ...
      case Z3_ROUNDING_MODE_SORT:
        return true;
      case Z3_ARRAY_SORT:// jfs add by zgf to support array
        return true;
      default: {
        // Sort not supported
        IF_VERB(ctx,
  ...
  if (cancelled) {                                                             \
    ...
    case LibFuzzerResponse::ResponseTy::TARGET_FOUND: {
      ...
      if (produceModel) {
        ...
        // add by zgf
        if (resp->model == nullptr){
          std::unique_ptr<CXXFuzzingSolverResponse> Unresp(
                  new CXXFuzzingSolverResponse(SolverResponse::UNKNOWN));
          return  Unresp;
        }
      }
      ...
      if (options->debugSaveModel) {
        ...
        // jfs add by zgf : if not have free var to assigned, don't need to
        // save buffer assignment
        if (ba->getRequiredStoreBytes() == 0)
          return resp;
