...
// A generator that fill the Dreal Solution as fuzzing seed

class DrealFuzzSeedGenerator : public SeedGenerator {
private:
    std::map<std::string,uint64_t> fuzzSeeds;
    bool seedWritten;

public:
    DrealFuzzSeedGenerator(llvm::StringRef name, std::map<std::string,uint64_t> &fuzzSeeds);
    void preGenerationCallBack(SeedManager& sm) override {}
    void postGenerationCallBack(SeedManager& sm) override {}
    bool writeSeed(SeedManager& sm) override;
    bool empty() const override;
