  ...
  with open(trend_file, 'r') as log:
    for line in log:
      if line.find(driver_tag) != -1:
        ...
        if len(line.split(',')) != 3:
          continue
        sec,coverage,covline = line.split(',')
        if len(sec) > 0:
          sec = int(sec)
...
def f_line(ax, dataX, dataY, marker, label, color, cname):
  ...
  ax.set_ylabel("#cLoC", fontsize=12)
  # ax.set_ylim(0, 20000)
  # # 设置横轴标签设置
  # NUM = 7
...
if __name__ == "__main__":
  ...
  class_name = ["solveEqu"]
  # class_name = ["elementary", "complex", "diffAndInteg", "cdf", "solveEqu", "compAndopt", "sf"]
  ...
  for cname in class_name:
    ...
    plt.savefig("fig1yx/" + cname + "_bfs_covtrend.pdf", dpi=300, bbox_inches='tight', pad_inches=0.1)
    ...
    # plt.show()
    plt.savefig("fig1yx/" + cname + "_dfs_covtrend.pdf", dpi=300, bbox_inches='tight', pad_inches=0.1)
