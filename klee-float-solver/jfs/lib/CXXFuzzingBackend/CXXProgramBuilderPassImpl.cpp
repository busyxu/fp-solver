...
CXXTypeRef CXXProgramBuilderPassImpl::getOrInsertTy(Z3ASTHandle declAPP) {
  ...
  case Z3_ARRAY_SORT:{ // jfs add by zgf : only use for array variable model
    // get the 'array variable' max select size
    unsigned width = arrayVarWidth[declAPP];

    std::string underlyingString;
    llvm::raw_string_ostream ss(underlyingString);
    ss << "BitVector<" << width << ">";
    ss.flush();
    auto ty = std::make_shared<CXXType>(program.get(),
                                        underlyingString,/*isConst=*/true);
    sortToCXXTypeCache.insert(std::make_pair(sort, ty));
    return ty;
  }
  default:
...
// jfs add by zgf to compute function hash to find correspond funcType
// NOTE !!! this function must correspond to 'klee::FunctionTypeInfo.uniq_name'
// in 'JsonParser.h'
std::string CXXProgramBuilderPassImpl::getUninterpretedFuncUniqName(
    jfs::core::Z3ASTHandle funcVar){
  //llvm::errs()<<"[zgf dbg] func str :"<<funcVar.asApp().getFuncDecl().toStr()<<"\n";
  std::string funcName = funcVar.asApp().getFuncDecl().getName();
  if (funcName.find("CF_") == 0)
    funcName = funcName.substr(3);
  unsigned retSize = funcVar.asApp().getSort().getWidth() / 8;

  //std::string uniqName = funcName + "_r" + std::to_string(retSize);
  std::string uniqName = funcName;

  for (unsigned idx = 0;idx < funcVar.asApp().getNumKids();idx++){
    Z3ASTHandle kid = funcVar.asApp().getKid(idx);
    unsigned argSize = kid.getSort().getWidth() / 8;
    uniqName += "_" + std::to_string(argSize);
  }
  return uniqName;
}

// jfs add by zgf to declare uninterpreted function
void CXXProgramBuilderPassImpl::insertUninterpretedFuncDecl() {
  std::string underlyingString;
  llvm::raw_string_ostream ss(underlyingString);
  std::set<std::string> insertedFuncDeclName;

  for (auto funcVar : uninterpretedFuncVar){
    std::string funcVarUniqName = getUninterpretedFuncUniqName(funcVar);
    if (insertedFuncDeclName.find(funcVarUniqName) != insertedFuncDeclName.end())
      continue;
    insertedFuncDeclName.insert(funcVarUniqName);

    funcIter = functionsTypeInfo.find(funcVarUniqName);
    assert(funcIter != functionsTypeInfo.end() &&
      "uninterpreter function type infomation not found !");
    klee::FunctionTypeInfo &funcInfo = funcIter->second;

    // such as "extern "C" int function(int arg0, double arg1);"
    underlyingString.clear();
    ss << "extern \"C\" " << funcInfo.ret_type.type_name <<" "
       << funcInfo.func_name << "(";
    std::vector<klee::ParameterTypeInfo> &paraList = funcInfo.args_type;
    for (unsigned idx = 0; idx < paraList.size(); idx ++){
      ss << paraList[idx].type_name<<" arg"<<std::to_string(idx);
      if (idx < paraList.size() -1)
        ss <<", ";
    }
    ss <<")";
    program->appendDecl(
...
void CXXProgramBuilderPassImpl::insertFreeVariableConstruction(
  ...
  for (const auto& be : ba) {
    // Add assignment
    auto assignmentTy = getOrInsertTy(be.declApp);
    ...
    case Z3_ARRAY_SORT:{ // jfs add by zgf for 'Array' variable model
      ss << "makeBitVectorFrom"
         << "<" << be.getTypeBitWidth() << ">(" << bufferRefName << ", "
         << currentBufferBit << ", " << endBufferBit << ")";
      break;
...
// jfs add by zgf : support 'const_arr' assignment
void CXXProgramBuilderPassImpl::insertConstantArrayAssignments(
    CXXCodeBlockRef cb,
    const std::vector<Z3ASTHandle>& constraints) {
  std::map<std::string,BvArray> arrayInfo;
  std::map<std::string,BvArray>::iterator arrayIt;
  std::vector<std::pair<Z3ASTHandle,std::string>> selectStringPair;

  for (auto const &constraint : constraints){
    Z3AppHandle e = constraint.asApp();
    if (e.getKind() != Z3_OP_EQ ||
        !e.getKid(0).isConstArraySelect()) continue;

    // 'equal' statement's first child must be 'select' statement,
    // 'equal' statement's second child must be constant number,
    // 'select' statement's first child name must begin with 'const_arr'
    Z3AppHandle selectStatement = e.getKid(0).asApp();
    Z3AppHandle constNumber = e.getKid(1).asApp();
    Z3AppHandle targetArray = selectStatement.getKid(0).asApp();
    std::string arrName = targetArray.toStr();

    uint64_t storeValue = 0, idxValue = 0;
    bool success = constNumber.getConstantAsUInt64(&storeValue);
    assert(success && "Failed to get storeValue");
    success = selectStatement.getKid(1).asApp().getConstantAsUInt64(&idxValue);
    assert(success && "Failed to get idxValue");
    int widValue = idxValue + 1;

    arrayIt = arrayInfo.find(arrName);
    if (arrayIt == arrayInfo.end()){
      Z3SortHandle rangeSort = Z3SortHandle(
          ::Z3_get_array_sort_range(ctx.getZ3Ctx(),
                                    targetArray.getSort()),
                                    ctx.getZ3Ctx());
      unsigned range = ::Z3_get_bv_sort_size(ctx.getZ3Ctx(), rangeSort);
      BvArray bvArr = BvArray(targetArray.asAST(),range);
      arrayInfo.insert(std::make_pair(arrName,bvArr));
    }
    arrayIt = arrayInfo.find(arrName);
    assert(arrayIt->second.width + 1 == widValue
           && "constraints must be not in sequence!");
    arrayIt->second.data.push_back((unsigned char)storeValue);
    // find the const_arr max select szie
    arrayIt->second.width = widValue;
    constArrayBytes[arrayIt->second.declApp] = arrayIt->second.getBytes();

    // 'select' const array statement is equal to constValue, such as
    // (select const_arr10 #x00000000) = BitVector<8>(UINT64_C(7));
    std::string selectString;
    llvm::raw_string_ostream se(selectString);
    se << "BitVector<" << arrayIt->second.range << ">(UINT64_C("<< storeValue << "))";
    se.flush();
    selectStringPair.emplace_back(selectStatement.asAST(),selectString);
  }

  for (auto &arrIt : arrayInfo){
    // const array assignment : const BitVector<160> const_arr
    BvArray array = arrIt.second;

    // get const array declare
    Z3SortHandle sort = array.declApp.getSort();
    auto assignmentTy = std::make_shared<CXXType>(
        program.get(), "BitVectorArray",/*isConst=*/false);
    sortToCXXTypeCache.insert(std::make_pair(sort, assignmentTy));

    // put const_array name into symbol table
    llvm::StringRef sanitizedSymbolName =
        insertSSASymbolForExpr(array.declApp, array.getName());

    // get const_array define statemnt
    std::string underlyingString;
    llvm::raw_string_ostream ss(underlyingString);
    underlyingString.clear();

    assert(array.declApp.getSort().getKind() == Z3_ARRAY_SORT &&
           "this function only deal with const array assignment.");
    ss << "BitVectorArray"
       << "(" << constArrayBytes[array.declApp] << ")";
    ...
    // push back const array all store statement
    for (int idx=0; idx < array.data.size();idx++){
      std::string storeString;
      llvm::raw_string_ostream ss(storeString);
      ss << array.name <<".store("<< idx<<","<<(int)array.data[idx]<<")";
      ss.flush();
      auto constArrayStoreStmt = std::make_shared<CXXGenericStatement>(
          cb.get(), storeString);
      //constArrayStoreStmt->print(llvm::errs());
      cb->statements.push_back(constArrayStoreStmt);
    }
  }

  for (auto const& stmtPair : selectStringPair)
    insertSSAStmt(stmtPair.first, stmtPair.second);
...
void CXXProgramBuilderPassImpl::build(const Query& q) {
  ...
  // jfs add by zgf to get functions type infomation
  // to support uninterpreter function
  functionsTypeInfo = q.functionsType;
  uninterpretedFuncVar = info->freeVariableAssignment->uninterpretedFuncVar;

  // jfs add by zgf to get array max select size
  arrayVarWidth = info->freeVariableAssignment->arrayVarWidth;

  insertHeaderIncludes();

  // jfs add by zgf to decalare used uninterpreted function name
  insertUninterpretedFuncDecl();
  ...
  // jfs add by zgf to support 'const_arr'
  insertConstantArrayAssignments(fuzzFn->defn,q.constraints);
...
// jfs add by zgf to transfer JFS TYPE to actual type use for call UF.
std::string CXXProgramBuilderPassImpl::JFS2ActualType(
    Z3ASTHandle e,
    const klee::ParameterTypeInfo& paraInfo,
    const std::string& argName){
  std::string jfsTypeName = getOrInsertTy(e)->getName();
  unsigned jfsWidth = e.getSort().getBitVectorWidth();

  std::string actualTypeName =  paraInfo.type_name;

  std::string underlyingString;
  llvm::raw_string_ostream ss(underlyingString);

  ss << actualTypeName <<" " << argName << " = ";

  if (jfsTypeName.find("BitVector") != std::string::npos){
    if (actualTypeName.find("float") != std::string::npos)
      ss << "Float<8,24>(" <<getSymbolFor(e) << ").data";
    else if(actualTypeName.find("double") != std::string::npos)
      ss << "Float<11,53>(" <<getSymbolFor(e) << ").data";
    else
      ss << getSymbolFor(e) <<".data";
  }else if (jfsTypeName.find("Float") != std::string::npos){
    if (actualTypeName.find("float") != std::string::npos ||
        actualTypeName.find("double") != std::string::npos){
      ss << getSymbolFor(e) <<".data";
    }else{
      assert("unsupport transform type ! check the usage of this function.");
    }
  }else{
    assert("unsupport jfs type ! check the usage of this function.");
  }
  ss.flush();
...
// jfs add by zgf to transfer actual type to JFS TYPE, for saving UF.
// note UF represent in JFS must be BitVector !
std::string CXXProgramBuilderPassImpl::Actual2JFSType(
    Z3ASTHandle e,
    const klee::ParameterTypeInfo& retInfo,
    const std::string& retValName,
    const std::string& newValName){
  std::string jfsTypeName = getOrInsertTy(e)->getName();
  unsigned jfsWidth = e.getSort().getBitVectorWidth();

  std::string actualTypeName =  retInfo.type_name;

  std::string underlyingString;
  llvm::raw_string_ostream ss(underlyingString);

  ss << "const "<< jfsTypeName <<" " << newValName << " = ";

  // Float32(256.2f).transferToBV<32>()
  if (jfsTypeName.find("BitVector") != std::string::npos){

    if (actualTypeName.find("float") != std::string::npos)
      ss << "Float32(" << retValName << ").transferToBV<"<<jfsWidth<<">()";
    else if(actualTypeName.find("double") != std::string::npos)
      ss << "Float64(" << retValName << ").transferToBV<"<<jfsWidth<<">()";
    else
      ss << jfsTypeName <<"(" << retValName << ")";
  }else if (jfsTypeName.find("Float") != std::string::npos){
    // Although i think this case may impossible to reach,
    // but i still want to descirbe it, maybe someone will use it.
    if (actualTypeName.find("float") != std::string::npos ||
        actualTypeName.find("double") != std::string::npos){
      ss << jfsTypeName <<"(" << retValName << ")";
    }else{
      assert(false && "unsupport transform type ! check the usage of this function.");
    }
  }else if (jfsTypeName.find("bool") != std::string::npos){
    ss <<  retValName ;
  }else {
    assert(false && "unsupport jfs type ! check the usage of this function.");
  }
  ss.flush();
...
// jfs add by zgf to support uninterpreted function
void CXXProgramBuilderPassImpl::visitUninterpretedFunc(Z3AppHandle e) {
  std::string funcVarUniqName = getUninterpretedFuncUniqName(e.asAST());
  funcIter = functionsTypeInfo.find(funcVarUniqName);
  assert(funcIter != functionsTypeInfo.end());

  klee::FunctionTypeInfo funcTypeInfo = funcIter->second;
  std::string callFuncString, paraString;

  // get each arg types assignment statement, such as :
  // int arg0 = bv2int(getSymbol(e.getKid(0)));
  // float arg1 = Float<8,24>(jfs_ssa_1).data;
  // double arg2 = Float<11,53>(jfs_ssa_2).data;
  std::string funcName = e.getFuncDecl().getName();
  std::vector<std::string> argList;
  for(unsigned i=0;i<e.getNumKids();i++){
    Z3ASTHandle arg = e.getKid(i);
    klee::ParameterTypeInfo paraInfo = funcTypeInfo.args_type[i];

    llvm::StringRef argNewName = insertSymbol(funcName +
                                              "_arg" + std::to_string(i));
    argList.push_back(argNewName);
    paraString += argNewName;
    if (i != e.getNumKids() - 1)
      paraString += ",";

    // transfer BV to actual type in "C", then use them to call UF.
    std::string argTransferString = JFS2ActualType(arg,paraInfo,argNewName);
    auto argAssignStmt = std::make_shared<CXXGenericStatement>(
        getCurrentBlock().get(), argTransferString);
    getCurrentBlock()->statements.push_back(argAssignStmt);

    /*std::string printStmtStr = "printf(\"[zgf dbg] arg val: 0x%02f \\n\",";
    printStmtStr += argNewName.str() + ")" ;
    auto printStmt = std::make_shared<CXXGenericStatement>(
        getCurrentBlock().get(), printStmtStr);
    getCurrentBlock()->statements.push_back(printStmt);*/
  }

  klee::ParameterTypeInfo retType = funcTypeInfo.ret_type;
  llvm::StringRef retName = insertSymbol(funcName + "_ret");

  // insert function call statement, but we must transfer retVal into BV/Float
  llvm::raw_string_ostream ss(callFuncString);
  ss << retType.type_name <<" " << retName << " = "
     << funcTypeInfo.func_name <<"(" << paraString<< ")";
  ss.flush();

  auto callFuncStmt = std::make_shared<CXXGenericStatement>(
      getCurrentBlock().get(), ss.str());
  getCurrentBlock()->statements.push_back(callFuncStmt);

  // save uninterpreter function expr correspond which symbol
//  auto statusPair = exprToSymbolName.insert(
//      std::make_pair(e.asAST(), retName));
//  assert(statusPair.second && "expr already has symbol");

//  std::string printStmtStr = "printf(\"[zgf dbg] UF ret: 0x%02f  \\n\",";
//  printStmtStr += retName.str() + ")" ;
//  auto printStmt = std::make_shared<CXXGenericStatement>(
//      getCurrentBlock().get(), printStmtStr);
//  getCurrentBlock()->statements.push_back(printStmt);

  // then we transfer retVal into BV, because all UF modeled with BV
  callFuncString.clear();
  std::string symbolName = getFreshSymbol();
  llvm::StringRef newName = insertSymbol(symbolName);

  // transfer actual type to JFS type, which value represent the call of UF.
  std::string saveUFstr = Actual2JFSType(e.asAST(),retType,retName,newName);

  auto saveUFStmt = std::make_shared<CXXGenericStatement>(
      getCurrentBlock().get(), saveUFstr);
  getCurrentBlock()->statements.push_back(saveUFStmt);

  // save uninterpreter function expr correspond which symbol
  auto statusPair = exprToSymbolName.insert(
      std::make_pair(e.asAST(), newName));
  assert(statusPair.second && "expr already has symbol");
}
...
// jfs add by zgf to support 'select'
void CXXProgramBuilderPassImpl::visitBvSelect(Z3AppHandle e) {
  // const array select statements have been dealed with before, don't care it
  if (e.isConstArraySelect()) return;

  assert(e.getNumKids() == 2);
  std::string underlyingString;
  llvm::raw_string_ostream ss(underlyingString);
  Z3ASTHandle targetArray = e.getKid(0);
  // jfs note by zgf : in KLEE Kquery select format, may be in some situation
  // it will happen some bugs.
  assert(targetArray.getSort().getKind() == Z3_ARRAY_SORT);
  Z3SortHandle rangeSort = Z3SortHandle(::Z3_get_array_sort_range(ctx.getZ3Ctx(),
                                          targetArray.getSort()),ctx.getZ3Ctx());
  unsigned range = ::Z3_get_bv_sort_size(ctx.getZ3Ctx(), rangeSort);

  Z3ASTHandle sizeAst = e.getKid(1);
  Z3SortHandle sizeSort = sizeAst.getSort();
  assert(sizeSort.isBitVectorTy());

  // if this 'Select' belong to 'FreeVar' select
  if(sizeAst.isConstant()) {
    // Get constant : must change constant 'arg1' to type 'app'
    uint64_t value = 0;
    Z3AppHandle asApp = sizeAst.asApp();
    // actually get value
    bool success = asApp.getConstantAsUInt64(&value);
    assert(success && "Failed to get numeral value");

    ss << getSymbolFor(targetArray) << ".select<" << range << ">("
       << range * value<< ", "
       << range * (1+value) - 1<< ")";

  }else {
    ss << getSymbolFor(targetArray) << ".select<" << range << ">("
       << range <<" * " << getSymbolFor(sizeAst)<< ".data, "
       << range <<" * (1 + " << getSymbolFor(sizeAst)<< ".data) - 1)";
  }
  insertSSAStmt(e.asAST(), ss.str());
}

// jfs add by zgf to support 'store' for array and bitVector
void CXXProgramBuilderPassImpl::visitBvStore(Z3AppHandle e) {
  //llvm::errs()<<"[zgf dbg] visitBvStore Expr:\n"<<e.toStr()<<"\n";
  assert(e.getNumKids() == 3);
  std::string underlyingString;
  llvm::raw_string_ostream ss(underlyingString);
  auto target = e.getKid(0);
  auto index = e.getKid(1);
  auto storeValue = e.getKid(2);

  if (storeValue.isConstant()) {
    // Get constant : must change constant 'arg1' to type 'app'
    uint64_t value = 0;
    Z3AppHandle asApp = storeValue.asApp();
    // actually get value
    bool success = asApp.getConstantAsUInt64(&value);
    assert(success && "Failed to get numeral value");
    ss << getSymbolFor(target) << ".store("
       << getSymbolFor(index)  <<".data, "<< value << ")";
  }else {
    ss << getSymbolFor(target) << ".store("
       << getSymbolFor(index)  <<".data, "
       << getSymbolFor(storeValue)  << ".data)";
  }
  // because store statement have not return value, so we use const array
  // name to repersent this 'expression'
  auto statusPair = exprToSymbolName.insert(
      std::make_pair(e.asAST(), getSymbolFor(target)));
  assert(statusPair.second && "expr already has symbol");

  // const array store don't need to return a new array
  auto storeStmt = std::make_shared<CXXGenericStatement>(
      getCurrentBlock().get(), ss.str());
  getCurrentBlock()->statements.push_back(storeStmt);
}
...
//  llvm::StringRef argNewName = getFreshSymbol();
//  auto assignmentTy = getOrInsertTy(e.asAST());
//  llvm::StringRef usedSymbol = insertSSASymbolForExpr(e.asAST(), argNewName);
//  auto assignmentStmt = std::make_shared<CXXDeclAndDefnVarStatement>(
//      getCurrentBlock().get(), assignmentTy, usedSymbol, ss.str());
//  getCurrentBlock()->statements.push_back(assignmentStmt);
//
//  std::string printStmtStr = "printf(\"[zgf dbg] BV2FP: 0x%02lx \\n\",";
//  printStmtStr += argNewName.str() + ".data)" ;
//  auto printStmt = std::make_shared<CXXGenericStatement>(
//      getCurrentBlock().get(), printStmtStr);
//  getCurrentBlock()->statements.push_back(printStmt);
}
...
#define FP_SPECIAL_CONST(NAME, CALL_NAME)                                      \
  void CXXProgramBuilderPassImpl::NAME(jfs::core::Z3AppHandle e) {             \
    ...
    auto sort = e.getSort();                                                   \
    assert(sort.isFloatingPointTy());                                          \
    auto cxxType = getOrInsertTy(e.asAST());                                   \
    std::string underlyingString;                                              \
    llvm::raw_string_ostream ss(underlyingString);                             \
...
// jfs add by zgf
void CXXProgramBuilderPassImpl::visitConvertToIEEEBitVectorFromFloat(
    jfs::core::Z3AppHandle e) {
  assert(e.getNumKids() == 1);
  auto arg = e.getKid(0);
  auto argSort = arg.getSort();
  assert(argSort.isFloatingPointTy());
  std::string underlyingString;
  auto resultSort = e.getSort();
  assert(resultSort.isBitVectorTy());
  auto cxxType = getOrInsertTy(e.asAST());
  llvm::raw_string_ostream ss(underlyingString);
  ss << getSymbolFor(arg) << ".transferToBV<"
     << resultSort.getBitVectorWidth() << ">()";
  insertSSAStmt(e.asAST(), ss.str());
}
