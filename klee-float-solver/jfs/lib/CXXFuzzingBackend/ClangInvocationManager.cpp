...
#define CHECK_CANCELLED()                                                      \
  if (cancelled) {                                                             \
    ...
    // jfs add by zgf to set target linked Libs
    for (const auto &libs : options->targetLinkedLibs)
      cmdLineArgs.push_back(libs.c_str());
    ...
    // jfs add by zgf to link test_no_main.o/runtime.lib
    // which has the implementation of UF
    //for (const auto &objs : options->pathToTargetNoMainObj)
    //  cmdLineArgs.push_back(objs.c_str());
    cmdLineArgs.push_back(options->pathToFPCheckLib.c_str());
