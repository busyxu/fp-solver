...
#define SET_ARG(NAME, X)                                                       \
  ...
  underlyingString.clear();
    ...
    // jfs add by zgf : to set max fuzzing time
    SET_ARG(maxTotalTimeArg, "-max_total_time=" << options->maxTotalTime);
    SET_ARG(useClangCoverageArg, "-use_clang_coverage=" << (options->useClangCoverage ? "1" : "0"));
    SET_ARG(useCountersArg, "-use_counters=" << (options->useCounters ? "1" : "0"));
    SET_ARG(useValueProfileArg, "-use_value_profile=" << (options->useValueProfile ? "1" : "0"));
...
#undef SET_ARG
    ...
    if (exitCode != targetFoundExitCode) {
      // jfs add by zgf : not found solution, be quient
