...
bool FreeVariableToBufferAssignmentPass::run(jfs::core::Query& q) {
  ...
  for (const auto& c : q.constraints) {
    // jfs add by zgf : skip constArray assignment equal
    if (c.asApp().getKind() == Z3_OP_EQ &&
        c.asApp().getKid(0).isConstArraySelect())
      continue;
  ...
  while (workList.size() > 0) {
    ...
    // jfs add by zgf : to get Array max Size from 'Select' operation
    Z3AppHandle nodeApp = node.asApp();
    if (nodeApp.getKind() == Z3_OP_SELECT){
      Z3ASTHandle var = nodeApp.getKid(0);
      Z3AppHandle selectSize = nodeApp.getKid(1).asApp();

      // such as : (declare-fun a0 () (Array (_ BitVec 32) (_ BitVec 8)))
      // we can not know how many bytes 'a0' is, so we must record max bytes.
      if (var.isArrayVariable()){
        // actually get value
        uint64_t value = 0;
        bool success = selectSize.getConstantAsUInt64(&value);
        assert(success && "Failed to get numeral value");
        Z3SortHandle range = Z3SortHandle(
            ::Z3_get_array_sort_range(ctx.getZ3Ctx(), var.getSort()),ctx.getZ3Ctx());
        unsigned bitWidth = ::Z3_get_bv_sort_size(ctx.getZ3Ctx(), range);
        uint64_t arrayWidth = bitWidth * (value + 1);

        //llvm::errs()<<"[zgf dbg] free var name:"<<
        //     var.toStr()<<"  value:"<<value<<"  size:"<<arrayWidth<<"\n";

        // record the max select width value
        arrayVarIter = arrayVarWidth.find(var);
        if (arrayVarIter == arrayVarWidth.end())
          arrayVarWidth.insert(std::make_pair(var,arrayWidth));
        else if (arrayVarIter->second < arrayWidth)
          arrayVarIter->second = arrayWidth;
      }
    }else if (node.asApp().getKind() == Z3_OP_UNINTERPRETED &&
        !node.isFreeVariable()){
      // jfs add by zgf to support uninterpreter function decalation
      uninterpretedFuncVar.push_back(node);
    }
  ...
  for (const auto& freeVarApp : orderedFreeVariableApps) {
    ...
    // jfs add by zgf : if this freeVar is 'constArray', don't alloc size for it.
    if (alreadyAssigned.count(freeVarApp) > 0 ||
        freeVarApp.isConstArray()) {
    ...
    if (equalitySetsIt == eep.mapping.end() &&
      ...
      // jfs add by zgf : to set Array BufferElement max size
      if (freeVarApp.asApp().getSort().isArrayTy()){
        uint64_t arrayWidth = arrayVarWidth[freeVarApp];
        el.setArrayVarWidth(arrayWidth);
      }
    ...
    // jfs add by zgf : don't evaluate const array
    if (freeVarApp.isConstArray()) continue;
