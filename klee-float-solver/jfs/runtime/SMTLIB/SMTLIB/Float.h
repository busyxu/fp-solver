...
// jfs modify by zgf to set 'data' from private to public
public:
  jfs_nr_float32 data;
  ...
  template <uint64_t BVWIDTH,
      typename = typename std::enable_if<
          (BVWIDTH <= JFS_NR_BITVECTOR_TY_BITWIDTH)>::type>
  BitVector<BVWIDTH> transferToBV() const {
    return BitVector<BVWIDTH>(jfs_nr_bitcast_float32_to_bv(data));
  }
...
public:
  ...
  template <uint64_t BVWIDTH,
      typename = typename std::enable_if<
          (BVWIDTH <= JFS_NR_BITVECTOR_TY_BITWIDTH)>::type>
  BitVector<BVWIDTH> transferToBV() const {
    return BitVector<BVWIDTH>(jfs_nr_bitcast_float64_to_bv(data));
  }
