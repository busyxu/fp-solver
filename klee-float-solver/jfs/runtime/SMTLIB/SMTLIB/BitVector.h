...
public:
  // jfs modify by zgf : move 'data' from private to public
  dataTy data;
  ...
  // jfs add by zgf to 'select' runtime
  template <uint64_t BITS>
  BitVector<BITS> select(uint64_t lowBit, uint64_t highBit) const {
    jfs_nr_bitvector_ty tempData = data;
    tempData = (tempData << (64 - (highBit + 1))) >> (lowBit + 64 - (highBit + 1));
//  printf("[zgf dbg] select bv: 0x%02lx  wid: %d\n",bv,bvWidth);
//  printf("   low:%d, high:%d, result : 0x%02lx \n",lowBit,highBit,data);
    return BitVector<BITS>(tempData);
  }
  ...
  // jfs add by zgf for debug dump data
  void dump() const{
    printf("[zgf dbg] BV dump data : 0x%02lx \n",data);
  }
...
// jfs add by zgf : model 'BitVectorArray',
// only have 2 operation : select/store
class BitVectorArray {
private:
  unsigned char* data; // range must be 8
  int width; // how many 'char' in array
  int bits; // bits = range * width

public:
  BitVectorArray(int bytes):
      width(bytes){
    bits = width << 3;
    data = (unsigned char*) calloc(width, sizeof(unsigned char));
  }

  template <uint64_t BITS>
  BitVector<BITS> select(uint64_t lowBit, uint64_t highBit) const {
    int startByte = (lowBit + 7) / 8;
    int endByte = (highBit + 7) / 8;
    unsigned offset = 0;

    //printf("[zgf dbg]  low:%d, high:%d\n",lowBit,highBit);
    jfs_nr_bitvector_ty result = 0;
    for (int idx = startByte;idx<endByte;idx++){
      result |= data[idx] << offset;
      offset += 8;
      //printf("   idx:%d,  data[idx]:0x%02lx, res:0x%02lx \n",idx,data[idx],result);
    }
    return BitVector<BITS>(result);
  }

  // note : 'index' here infer to array.data[index]
  // TODO: this method maybe have problem in some case.
  void store(int index, unsigned char storeData){
    //printf("[zgf dbg] array store idx:%d, data:%d\n",index,storeData);
